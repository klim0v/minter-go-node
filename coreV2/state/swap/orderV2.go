...
var burnAddress = types.HexToAddress("Mx00cedde786b34d733d1dc96559253081572df2c6")
...
func (s *SwapV2) loadBuyOrders(pair *PairV2, fromOrder *Limit, limit int) []uint32 {
	...
	if fromOrder != nil {
		log.Println("fromOrder", fromOrder.id)
...
func (p *PairV2) orderSellLoadToIndex(index int) *Limit {
	...
	if len(orders) != 0 {
		...
		if p.hasUnsortedSellOrders() || p.hasDeletedSellOrders() {
			...
			needLoadMore := len(orders) - len(p.deletedSellOrderIDs().list)
			...
			if needLoadMore > 0 { // needLoadMore == 0 ignore ok
				log.Println("needLoadMore", needLoadMore)
			...
			if lastI >= 0 && orders[lastI] != 0 {
				...
				if index > lastI {
					...
					fromOrder = p.order(orders[lastI])
					log.Println("index > lastI", index-lastI)
